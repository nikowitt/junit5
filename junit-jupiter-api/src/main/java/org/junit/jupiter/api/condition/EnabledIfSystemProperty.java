...
@Documented
@ExtendWith(EnabledIfSystemProperty.Condition.class)
	...
	class Condition implements ExecutionCondition {
		...
		public ConditionEvaluationResult evaluateExecutionCondition(ExtensionContext context) {
			...
			if (actual.matches(regex)) {
				return enabled(format("System property [%s] with value [%s] matches regular expression [%s]", name,
					actual, regex));
			}
			// else
			return disabled(format("System property [%s] with value [%s] does not match regular expression [%s]", name,
				actual, regex));
		}
